...
export async function fetchSubCategories(categoryId?: string): Promise<SubCategory[]> {
  ...
  const data = await response.json();
  if (!data.documents) return [];
  let results: SubCategory[] = data.documents.map((doc: any) => {
    const f = doc.fields;
    return {
      id: doc.name.split('/').pop() as string,
...
export async function fetchTopics(subCategoryId?: string): Promise<Topic[]> {
  ...
  const data = await response.json();
  if (!data.documents) return [];
  let results: Topic[] = data.documents.map((doc: any) => {
    const f = doc.fields;
    return {
      id: doc.name.split('/').pop() as string,
      ...
      questionCount:
        ...
        Number(f.questionCount?.doubleValue) ||
        parseInt(f.questionCount?.stringValue || '0', 10) ||
        0,
    };
  });
  if (subCategoryId) {
