...
export const sessionManager = {
  ...
  markQuestionsSeen(session: GuestSession, scopeKey: string, questionIds: string[]): GuestSession {
    if (!session.topicsSeen[scopeKey]) {
      session.topicsSeen[scopeKey] = [];
    }
    const newIds = questionIds.filter(id => !session.topicsSeen[scopeKey].includes(id));
