...
const latestPosts = [
  { id: '1', title: 'Understanding Heart Failure Management', topic: 'Cardiovascular', readTime: '5 min read', date: '2 days ago' },
  { id: '2', title: 'NCLEX Pharmacology Tips: Must-Know Drug Classes', topic: 'Pharmacology', readTime: '7 min read', date: '3 days ago' },
  { id: '3', title: 'Clinical Assessment: Respiratory System Guide', topic: 'Respiratory', readTime: '6 min read', date: '5 days ago' },
];

const categories = [
  { icon: '🏥', label: 'Clinical Medicine', color: 'hover:border-blue-300' },
  { icon: '🩺', label: 'NCLEX', color: 'hover:border-purple-300' },
  { icon: '🇳🇬', label: 'NMCN', color: 'hover:border-green-300' },
];
    ...
    <div className="space-y-10">
      ...
      <div className="grid grid-cols-3 gap-4 max-w-lg mx-auto">
        {categories.map(cat => (
          <div
            key={cat.label}
            className={`text-center p-5 bg-white rounded-xl border-2 border-gray-200 hover:shadow-md transition-all cursor-pointer ${cat.color}`}
          >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-4xl mx-auto">
          {features.map(feature => (
            <Link
              key={feature.title}
              to={feature.link}
              className={`p-6 rounded-xl border-2 transition-all duration-200 hover:shadow-md ${feature.color}`}
            >
      ...
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center justify-between mb-4">
          ...
          <Link to="/feed" className="text-sm text-primary-600 font-medium hover:underline">
            View all →
          </Link>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {latestPosts.map(post => (
            ...
            >
              ...
              <h3 className="text-sm font-semibold text-gray-900 mt-3 mb-2 line-clamp-2">{post.title}</h3>
      ...
      <div className="text-center pb-4">
        <Link to="/rapid-quiz" className="btn-primary inline-block text-lg px-8 py-3 shadow-lg hover:shadow-xl">
          Start Practice Now
        </Link>
